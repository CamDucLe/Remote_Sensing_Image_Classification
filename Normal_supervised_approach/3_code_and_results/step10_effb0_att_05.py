...
def main():
    ...
    with strategy.scope():
        ...
        if os.path.isfile(best_model_file):
            ...
            block4_x = base_model.get_layer('block4c_add').output  
            block3_x = base_model.get_layer('block3b_add').output  


            #print('# block 3: ', np.shape(block3_x)) #  31,31,40
            #print('# block 4: ', np.shape(block4_x)) #  16,16,80
            #print('# block 5: ', np.shape(block5_x)) #  16,16,112
            #print('# block 6: ', np.shape(block6_x)) #  8,8,192
            #print('# block 7: ', np.shape(block7_x)) #  8,8,1280
            #exit()
            ...
            block4_x = Conv2D(filters=1280, kernel_size=2, strides=2)(block4_x) 
            block3_x = Conv2D(filters=1280, kernel_size=4, strides=4)(block3_x) 

            block7_x = trippleAttention(block7_x)
            block6_x = trippleAttention(block6_x)
            block5_x = trippleAttention(block5_x)
            block4_x = trippleAttention(block4_x)
            block3_x = trippleAttention(block3_x)
            ...
            x = block3_x +  block4_x + block5_x + block6_x + block7_x
            x = Dropout(0.2)(x)
